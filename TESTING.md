...
8. A signed-in member can join a welcome table from Circle View's dashed "Join a welcome table" card and lands in that table's real feed.

The post-signup wizard used by every sign-in helper now has three steps (intro, profile, and a "find your way in" step offering a welcome table) - `e2e/support/auth.ts` is the single place that clicks through all three, so a future wizard change only needs updating there.
