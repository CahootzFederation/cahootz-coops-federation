...
20. A circle member picks their per-circle notification level from circle settings (`circle-notifications.spec.ts`): "Only @mentions" is the default; switching to "All activity" persists across reload and User A's next circle post creates a "posted in <circle>" alert for User B; switching to "Nothing" persists and User A's next post creates no alert.
21. A member edits their Personal Page profile (`personal-page-profile.spec.ts`): from "Add a bio"/"Edit profile" they pick a photo (the sheet previews it), switch to an emoji avatar, write a bio, and save; the bio and emoji persist after reload and User B sees both on the member's public `/people/<handle>` page. The spec restores the member's original bio and avatar afterwards. The photo is previewed but not uploaded, so the journey needs no blob-storage secret.
22. Signed-out visitors don't see the Shop tab or the drawer's stores entry, a direct `/store` link opens sign-in, and Circle View shows a Sign in card in place of the welcome lounge (`store-marketplace.spec.ts`, `circle-view.spec.ts`).
23. When Stripe test credentials and a payment-ready test Connect account are configured, the same journey buys the Seed Supporter badge through the real Stripe Payment Element and verifies payment confirmation. This journey is skipped when the dedicated Stripe secrets are absent; it must be enabled for staging release qualification.
24. A circle with an emoji icon renders on Circle View with the emoji's full glyph visible (its line box is at least as tall as its font size, so the top and bottom aren't clipped).
