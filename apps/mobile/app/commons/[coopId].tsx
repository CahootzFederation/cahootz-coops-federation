...
export default function CommonsDetailScreen() {
  ...
  const charterPreview =
    activeConfig.charterText.length > 520
            ...
            <>
              <View className="mb-3 flex-row gap-2">
                <View className="flex-1 rounded-xl border border-gray-200 bg-white p-3">
                  ...
                  <Text className="mt-2 text-xs font-bold uppercase text-gray-500">
                    Cash
                  </Text>
                  <Text className="mt-1 text-lg font-black text-gray-950">
                    {isMember ? formatMoney(undefined) : 'Locked'}
                  </Text>
                </View>
                <View className="flex-1 rounded-xl border border-gray-200 bg-white p-3">
                  ...
                  <Text className="mt-1 text-lg font-black text-gray-950">
                    {isMember ? proposals.length : 'Locked'}
                  </Text>
                </View>
                <View className="flex-1 rounded-xl border border-gray-200 bg-white p-3">
                  ...
                  <Text className="mt-1 text-lg font-black text-gray-950">
                    {formatPercent(activeConfig.approvalThresholdPercent)}
                  </Text>
                </View>
              </View>
              ...
              <View className="mb-3 rounded-2xl border border-gray-200 bg-white p-4">
                ...
                {activeConfig.missionGoals.length > 0 ? (
                    ...
                    {activeConfig.missionGoals.map((goal) => (
                      ...
                      >
                        <View className="flex-row items-start justify-between gap-3">
                          <Text className="min-w-0 flex-1 font-black text-gray-900">
                            {goal.label}
                          </Text>
                          <Text
                            className="text-xs font-black"
                  ...
                  <Text className="text-sm leading-5 text-gray-600">
                    No goals have been published for this commons yet.
                  </Text>
                )}
              </View>
                ...
                {isMember ? (
                  <>
                    ...
                    >
                      ...
                      <Text className="mt-1 text-sm leading-5 text-gray-700">
                        This page is ready for a per-commons cash balance once
                        treasury accounting is exposed to members.
                      </Text>
                    </View>
                    <View className="mt-3 gap-2">
                      <View className="flex-row justify-between gap-3">
                        ...
                        <Text className="flex-1 text-right text-sm font-black text-gray-900">
                          {shortAddress(activeConfig.treasurySafeAddress)}
                        </Text>
                      </View>
                      <View className="flex-row justify-between gap-3">
                        <Text className="text-sm font-semibold text-gray-500">
                        ...
                        <Text className="text-sm font-black text-gray-900">
                          {formatMoney(activeConfig.aiAutoApproveThresholdUSD)}{' '}
                          and under
                        </Text>
                      </View>
                      <View className="flex-row justify-between gap-3">
                        ...
                        <Text className="text-sm font-black text-gray-900">
                          {formatMoney(activeConfig.councilVoteThresholdUSD)}+
                        </Text>
                      </View>
                    </View>
                  </>
                  ...
                  <View className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-4">
                    <Text className="text-base font-black text-gray-900">
                      Members see treasury details
                    </Text>
                    <Text className="mt-1 text-sm leading-5 text-gray-600">
                      Cash balance, funding lanes, and safe details are visible
                      after joining this commons.
                    </Text>
                  </View>
                )}
              </View>
              ...
              <View className="mb-3 rounded-2xl border border-gray-200 bg-white p-4">
                ...
                {!isMember ? (
                  <View className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-4">
                    <Text className="text-base font-black text-gray-900">
                      Proposal room locked
                    </Text>
                    <Text className="mt-1 text-sm leading-5 text-gray-600">
                      You can apply from this page, then come back to see
                      proposal history, votes, and funding requests.
                    </Text>
                  </View>
                ) : proposals.length > 0 ? (
                  <View className="gap-3">
                    {proposals.map((proposal) => (
                      <TouchableOpacity
                        ...
                        onPress={() =>
                          router.push(
                            `/(tabs)/proposal-detail?id=${proposal.id}` as any,
                          )
                        }
                        className="rounded-xl border border-gray-100 bg-gray-50 p-3"
                        activeOpacity={0.75}
              ...
              <View className="mb-3 rounded-2xl border border-gray-200 bg-white p-4">
                ...
                {!isMember ? (
                  ...
                  <View className="gap-3">
                    {circles.slice(0, 3).map((circle) => (
                      <TouchableOpacity
                        ...
                        onPress={() =>
                          router.push({
                            ...
                            params: { coopId, circleId: circle.id },
                          } as any)
                        }
                      ...
                      >
                        ...
                        <View className="min-w-0 flex-1">
                          <Text
                            className="font-black leading-5 text-gray-900"
                            numberOfLines={1}
                          >
                            {circle.name}
                          </Text>
                  ...
                  <View className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-4">
                    ...
                    <Text className="mt-1 text-sm leading-5 text-gray-600">
                      Circles are small, private or invite-only spaces inside{' '}
                      {name} — good for a project team or a working group before
                      it needs a full proposal.
                    </Text>
                  </View>
                )}
              </View>
