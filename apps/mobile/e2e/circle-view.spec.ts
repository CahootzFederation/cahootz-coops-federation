...
// cover that navigation change plus the new welcome-table workflow, both
...
test("a signed-in member can join a welcome table from Circle View", async ({
  browser,
}) => {
  const { context, page } = await newSignedInPage(browser, USER_A_EMAIL);
  ...
  try {
    ...
    const alreadyMember = page.getByText(/^Welcome Table \d+$/);
    if (await alreadyMember.isVisible().catch(() => false)) {
      await alreadyMember.click();
    } else {
      await page.getByText("Join a welcome table", { exact: true }).click();
    }

    await expect(
      page.getByRole("textbox", { name: "Share what's happening..." }),
    ).toBeVisible();
    await expect(page.getByText(/Welcome Table \d+/).first()).toBeVisible();

    await page.getByLabel("Back to Circle View").click();
    await expect(page.getByText(/^Welcome Table \d+$/)).toBeVisible();
  } finally {
    await context.close();
  }
});
