...
test('member chooses a funding badge, opens the cart, and reaches checkout', async ({ browser }) => {
  ...
  try {
    ...
    await expect(member.page.getByText(/Continue to payment \$\d/)).toBeVisible();
  } finally {
    await member.context.close();
  }
});
...
test('funding badge checkout uses Stripe test mode when a ready official shop is configured', async ({ browser }) => {
  // Disabled in CI for now; opt in locally with E2E_ENABLE_STRIPE_JOURNEY=1.
  test.skip(process.env.E2E_ENABLE_STRIPE_JOURNEY !== '1', 'Stripe payment journey is temporarily disabled');
  ...
  try {
    await member.page.getByRole('tab', { name: 'Shop' }).click();
    await member.page.getByRole('tab', { name: 'stores' }).click();
    await expect(member.page.getByLabel('Open Cahootz Funding Shop')).toBeVisible();
    await member.page.getByRole('tab', { name: 'products' }).click();
    await member.page.getByLabel('Add Seed Supporter to cart').click();
    await member.page.getByLabel('Shopping cart').click();
    await expect(member.page.getByText('Subtotal', { exact: true })).toBeVisible();
    ...
    await expect(member.page.getByText(/Continue to payment \$\d/)).toBeVisible();
    await member.page.getByText(/Continue to payment \$\d/).click();

    await member.page.getByPlaceholder('Name on card...').fill('E2E Buyer');
    // Stripe renders more than one "Secure payment input frame"; use the one
    // that actually holds the card fields.
    let stripeFrame: Frame | undefined;
    await expect.poll(async () => {
      for (const frame of member.page.frames()) {
        if (await frame.locator('input[name="number"]').count().catch(() => 0)) stripeFrame = frame;
      }
      return !!stripeFrame;
    ...
    const postal = stripeFrame!.getByLabel(/zip|postal/i);
    ...
    await member.page.getByText(/^Pay \$\d/).click();
    expect(await confirmation).toContain('Payment Confirmed');
    await expect(member.page).toHaveURL(/order-detail\?id=/);
  } finally {
    await member.context.close();
  }
});
