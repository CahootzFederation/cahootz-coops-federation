...
export default function CommonsAIPage() {
  const { coopId } = useParams<{ coopId: string }>();
  const [data, setData] = useState<Dashboard | null>(null);
  const [drafts, setDrafts] = useState<Record<string, string>>({});
  const [error, setError] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  const endpoint = `/api/admin/commons/${encodeURIComponent(coopId)}/ai`;
  const refresh = useCallback(async () => {
    const response = await fetch(endpoint);
    const body = await response.json();
    if (!response.ok) throw new Error(body.error || "Could not load AI actions");
    setData(body);
  }, [endpoint]);
